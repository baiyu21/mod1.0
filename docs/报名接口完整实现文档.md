...
4. **上传作品**
   - 统一使用 `FileUploadBlock` 组件
   - 文件选择后自动上传并在控制台输出响应
   - 上传成功后显示“上传成功”标识
   - 支持音频、视频、图片、PDF等多种格式（≤100MB）
...
舞蹈报名 `EntryFormDance.vue` 与器乐报名结构一致，核心差异在于字段映射、默认值以及接口路径。以下仅列出不同之处：

### 1. 上传与表单

- `FileUploadBlock` 设置 `ref="fileUploadRef"`，并传入 `:upload-category="'dance'"`，选择文件后自动上传并显示“上传成功”标识。
- `mapPerformanceType` 将 `group-dance` 映射为后端 `group`，`mapGroupType` 将 `group1/group2` 映射为 `amateur/professional`。
...
const transformGuideTeachers = (teachers: RosterItem[]) => {
  return teachers
    ...
    .map(t => {
      const ethnicity = String(t.nation || '').trim() === '汉' ? '汉族' : (t.nation || '汉族')
      return {
        name: String(t.name || '').trim(),
        id_card_number: String(t.idNo || '').trim() || '000000000000000000',
        ethnicity,
        age: Number(t.age) || 35,
        gender: t.gender === 'male' ? 'male' : 'female',
        region: String(t.region || '').trim() || '北京市',
        school_name: String(t.school || '').trim() || '未填写',
        department: String(t.org || t.dept || '').trim() || '未填写',
        phone: String(t.phone || '').trim() || '13800000000'
      }
    })
}
```

参赛人员转换逻辑与器乐一致，只是默认年级（大二）和专业名称（舞蹈表演）更贴合舞蹈场景。
...
- 文件选择后自动上传，成功会在页面提示并输出日志
- `fileUploadRef.getFirstFileUrl()` 可读取已上传 URL
- 若仍未获取到 URL，再使用默认地址兜底
