...
const accepts = '.jpg,.jpeg,.png,.raw,.dng,.pdf'
...
const transformAuthors = (rows: RosterItem[]): Record<string, string>[] => {
  return rows
    ...
    .map(row => {
      ...
      authorData.major_name = major || '摄影专业'
...
/**
 * 构建 API 数据
 * @param workUrl 照片URL（暂存时可以为空或默认值）
 * @param status 状态：'draft' 暂存 或 'pending' 提交
 */
const buildApiData = (workUrl: string, status: 'draft' | 'pending') => {
  ...
  return {
    ...
    creation_time: String(baseForm.createAt || '').trim(),
    work_form: mapWorkForm(baseForm.formType),
    width: Number(baseForm.width) || 0,
    length: Number(baseForm.length) || 0,
...
const onSave = async () => {
  ...
  // 暂存时不需要上传文件，使用默认值
  const workUrl = 'https://example.com/sample_photography_photo.jpg'

  // 构建 API 数据，status 为 'draft'
  const apiData = buildApiData(workUrl, 'draft')

  // 调用后端接口暂存
  try {
    const success = await registrationApi.submitPhotography(apiData)
    if (success) {
      ...
      try {
        ...
        localStorage.setItem('photographyFormDraft', JSON.stringify(saveData))
      } catch (error) {
...
const onSubmit = async () => {
  ...
  // 先上传文件
  let workUrl = 'https://example.com/sample_photography_photo.jpg'
  ...
  const apiData = buildApiData(workUrl, 'pending')

  try {
    console.log('[PhotographySubmit] payload:', JSON.stringify(apiData, null, 2))
    const success = await registrationApi.submitPhotography(apiData)
    if (success) {
  ...
  <div class="ef-form">
    <el-card shadow="never" class="intro-card">
      ...
      <div class="intro-text">学生艺术作品需提交400以内的创作说明（包括作品主题简介和创作过程介绍）。<br>
        单张照和组照（每组不超过4幅，须标明序号）尺寸均为14英寸（30.48cmX35.36cm）；除影调处理外，不得利用电脑和暗房技术改变影像原貌。
      </div>
    </el-card>
          ...
          <el-col :span="12">
            <el-form-item label="作品形式" prop="formType">
              <el-select v-model="baseForm.formType" placeholder="请选择" style="width: 100%">
                <el-option label="单张照" value="single-photo" />
                <el-option label="组照" value="group-photo" />
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
    ...
    <el-card shadow="never" class="ef-section-card">
      ...
      <FileUploadBlock
        ...
        :limit="12"
        :readonly="readonly"
        upload-category="photography"
        tip="支持 jpg/png/raw/dng/pdf，单张建议≥3000px。"
